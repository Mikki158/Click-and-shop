...
const CartProduct = ({ product }: { product: ProductProps }) => {
  ...
  useEffect(() => {
    const getProductInfo = async () => {
      try {
        const responseBrand = await apiClient.get(`/api/seller/brand/${product.brandId}`);
        setBrand(responseBrand.data.brandName);
    
        const responseCategory = await apiClient.get(`/api/category/${product.categoryId}`);
      ...
      <div className="ml-4 flex flex-1 flex-col justify-between sm:ml-6">
        <div className="relative pr-9 sm:grid sm:grid-cols-4 sm:gap-x-6 sm:pr-0">
          <div className="flex flex-col gap-1 col-span-3">
            ...
            <p className="text-xs">
              Бренд: <span className=" font-medium">{brand}</span>
            </p>
            <p className="text-xs">
            ...
            <div className="flex items-center gap-6 mt-2">
              <p className="text-base font-semibold">
                <FormattedPrice
                  ...
                  //amount={product?.discountedPrice * 1}
                />
              </p>
